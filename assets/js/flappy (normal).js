...
/*criar FUNÇÃO para sortear abertura das BARREIRAS*/
    ...
    this.setX(eixoX)
/*chamada de sorteio de aberturas)*/
    this.sortearAbertura()
}
...
function GestorBarreiras(alturaGame,larguraGame,aberturaBarreira,distanciaBarreira,deslocamento,pontos){
    ...
    this.barreiras = []
/*calculo matemática colocar as barreiras no lugar delas*/
...
/* LEMBRE= quem se movem são as BARREIRAS e não o passaro*/
    this.getY = () =>{
...
this.animar = () =>{
    ...
    let val = 0;
    if(this.voando == 'subir'){
        val = 5;
    }else if(this.voando == 'decer'){
        val = -5;
    }
...
/*Definir a ALTURA e a LARGURA do GAME*/
    ...
    this.start = () => {
        const temporizador = setInterval(() =>{
            ...
            if(chekColisao(passaro, gestorBarreiras)){
                ...
                if(confirm("Você perdeu com um total de "+pontos+"pontos. Gostaria de jogar novamente?")){
     ...
     window.onkeydown = (e) =>{
        ...
        passaro.voando = "reto"
        if(e.keyCode == 38){
            passaro.voando = 'subir'
        }else if(e.keyCode == 40){
            passaro.voando = 'decer'
        }
       /*p=80 up=38  down=40*/
